...
WORKDIR /usr/local/flights-analytics
COPY . /usr/local/flights-analytics/

RUN echo "Building jar..." &&\
export SBT_OPTS="$SBT_OPTS -Xmx7G -Xss2M -XX:MaxMetaspaceSize=1024M" &&\
sbt "it:testOnly" "test:testOnly" "assembly" &&\
#sbt "assembly" &&\
chmod -R a+rw /usr/local/flights-analytics
...
COPY --from=builder /usr/local/flights-analytics/scripts /opt/scripts
COPY --from=builder /usr/local/flights-analytics/src/main/resources/log4j.properties $SPARK_HOME/conf/log4j.properties
COPY --from=builder /usr/local/flights-analytics/target/scala-2.12/flights-analytics-assembly-0.0.1-SNAPSHOT.jar $SPARK_HOME/

# storage for data TODO: later use/integrate with Minio
